...
require_once('ZmiennePolaczenia.php');
  ...
  if (isset($_POST['submit'])) {
    ...
    if (!$error) {
      if (!empty($AUTOR) && !empty($TYTUL) && !empty($CYKL) && !empty($TOM) && !empty($TLUMACZENIE) && !empty($TYTUL_ORG) && !empty($isbn) && !empty($LICZBA_STRON) && !empty($KATEGORIA) && !empty($JEZYK) && !empty($OPIS)/*potem dodać $OPIS*/){
        ...
        if (!empty($nowy_obraz)){
          ...
          $query = "UPDATE ksiazki SET Autor = '$AUTOR', Tytul = '$TYTUL', Cykl = '$CYKL', Tom = '$TOM', Tlumaczenie = '$TLUMACZENIE', TytulOrg = '$TYTUL_ORG', ISBN='$isbn', LStron='$LICZBA_STRON', Kategoria='$KATEGORIA', Jezyk='$JEZYK', Opis='$OPIS', Okladka = '$nowy_obraz' WHERE ID_ksiazki = '" . $_GET['ID_ksiazki'] . "'";
        }
        else{
			
            $query = "UPDATE ksiazki SET Autor = '$AUTOR', Tytul = '$TYTUL', Cykl = '$CYKL', Tom = '$TOM', Tlumaczenie = '$TLUMACZENIE', TytulOrg = '$TYTUL_ORG', ISBN='$isbn', LStron='$LICZBA_STRON', Kategoria='$KATEGORIA', Jezyk='$JEZYK', Opis='$OPIS', Okladka = '$stary_obraz' WHERE ID_ksiazki= '" . $_GET['ID_ksiazki'] . "'";
        }
		...
		if($error2 == false)
		{
			...
			echo '<p>Aktualizacja profilu zakończyła się sukcesem. Czy chcesz <a href="KsiazkaKonkretna.php?ID_ksiazki='.$_GET['ID_ksiazki'].'">zobaczyć zmiany?</a>?</p>';
		}
    ...
    $queryy = "SELECT Autor, Tytul, Cykl, Tom, Tlumaczenie, TytulOrg, ISBN, LStron, Kategoria, Jezyk, Opis, Okladka FROM ksiazki WHERE ID_ksiazki ='" . $_GET['ID_ksiazki'] . "'";
    $dataa = mysqli_query($dbc, $queryy);
    ...
    $CYKL = $roww['Cykl'];
    $TOM = $roww['Tom'];
    $TLUMACZENIE = $roww['Tlumaczenie'];
    $TYTUL_ORG = $roww['TytulOrg'];
	$isbn = $roww['ISBN'];
	$LICZBA_STRON = $roww['LStron'];
	$KATEGORIA = $roww['Kategoria'];
	$JEZYK = $roww['Jezyk'];
	$OPIS = $roww['Opis'];
    $stary_obraz = $roww['Okladka'];
